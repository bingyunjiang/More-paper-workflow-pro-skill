...
- For curves, evidence pixel gaps remain in `observations.csv`; only user-confirmed `curve_topology` controls formal breaks in style-independent `data.csv`. Any `guide_constrained` densification is written into `data.csv` with guide/residual/hash lineage, never generated by render.
- Official XLSX/CSV values may validate an immutable image-derived extraction but must not silently overwrite image-derived candidates, observations, or formal lineage.
